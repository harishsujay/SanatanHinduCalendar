...
class DetailArticleListItemDetails {
    ...
    let allArticles = [ArticleListItemDetails(
    ...
    func getAllArticlesByPageType (pageType:String)->[ArticleListItemDetails]{
        ...
        for index in 0...(self.allArticles.count-1) {
            //adding to the resultant array only if the type of the article is same as that of the page type passed
            if ( self.allArticles[index].aritcleType == pageType ){
                
                articleArray.append(self.allArticles[index]);
